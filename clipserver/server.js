...
app.post('/api/youtube/clip', async (req, res) => {
  ...
  try {
    ...
    const ytCmd = `"${ytdlpPath}" --force-ipv4 --no-check-certificates --extractor-args "youtube:player_client=tv" -f "best[ext=mp4]/best" --download-sections "*${start}-${end}" -o "${downloadPath}" "${youtubeUrl}"`;
    ...
    await new Promise((resolve, reject) => {
      exec(ytCmd, { timeout: 120000, env: execEnv }, (error, stdout, stderr) => {
        if (error) {
          ...
          const fallbackCmd = `"${ytdlpPath}" --force-ipv4 --no-check-certificates --extractor-args "youtube:player_client=tv" -f "best[height<=720][ext=mp4]/best" -o "${downloadPath}" "${youtubeUrl}"`;
