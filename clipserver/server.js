...
async function downloadYoutubeWithFallback(youtubeUrl, outputPath, startSec = null, endSec = null) {
  ...
  for (const client of clients) {
    try {
      ...
      const cmd = `"${ytdlpPath}" ${cookiesArg} --js-runtimes "node:${nodeBin}" --geo-bypass --no-check-certificates ${sectionArg} --extractor-args "youtube:player_client=${client}" --user-agent "${userAgent}" -f "best[height<=480][ext=mp4]/best[height<=720][ext=mp4]/best" -o "${outputPath}" "${youtubeUrl}"`;
